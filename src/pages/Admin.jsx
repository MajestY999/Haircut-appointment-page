...
// компонент ввода времени с нормализацией
...
export default function Admin() {
  ...
  const [schedules, setSchedules] = useState({}); // map: barberId -> schedule
  ...
  const addService = (svc) => {
    const list = [...services, svc].sort((a, b) => a.name.localeCompare(b.name));
    setServices(list);
  ...
  // форма услуг
  const [svcForm, setSvcForm] = useState({ id: null, name: '', price: '', duration: '' });
  const startAddService = () => setSvcForm({ id: null, name: '', price: '', duration: '' });
  const startEditService = (s) => setSvcForm({
    id: s.id,
    name: s.name || '',
  ...
  const submitServiceForm = (e) => {
    ...
    const duration = Number(svcForm.duration);
    if (!name) return;
    if (Number.isNaN(price) || Number.isNaN(duration)) return;

    if (svcForm.id == null) addService({ id: Date.now(), name, price, duration });
    else updateService(svcForm.id, { name, price, duration });

    setSvcForm({ id: null, name: '', price: '', duration: '' });
    reloadServices();
  };

  // дни недели (отображение Пн–Вс, хранение Mon..Sun)
  const daysOfWeek = [
              ...
              <div className={styles.actions}>
                ...
                {svcForm.id && (
                  <button
                    ...
                    type="button"
                    onClick={() => setSvcForm({ id: null, name: '', price: '', duration: '' })}
                  >
                    Отмена
                  </button>
                )}
                ...
                services.map(s => (
                  <div key={s.id} className={styles.serviceRow}>
                    <div>
                      ...
                      <div className={styles.label}>{s.price} ₽ — {s.duration} мин</div>
                    </div>
